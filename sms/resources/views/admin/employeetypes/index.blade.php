        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    ...
                    @if(Auth::user()->can('access.employeetypes.create'))
                        <a href="{{ url('/admin/employeetypes/create') }}" class="btn btn-success btn-sm pull-right" title="Add New Employeetypes">
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach($employeetypes as $item)
                                <tr>
                                    <td>{{ $item->id }}</td>
                                    <td><a href="{{ url('/admin/employeetypes', $item->id) }}">{{ $item->name }}</a></td>
                                    <td>
                                        <a href="{{ url('/admin/employeetypes/' . $item->id) }}" title="View Role">
                                            ...
                                            @if(Auth::user()->can('access.employeetypes.edit'))
                                                <a href="{{ url('/admin/employeetypes/' . $item->id . '/edit') }}"
                                            ...
                                            @if(Auth::user()->can('access.employeetypes.delete'))
                                                <form method="POST" action="{{ url('/admin/employeetypes'.'/'.$item->id) }}" accept-charset="UTF-8" style="display:inline" class = "confirm_delete">
                                                    {{ method_field('DELETE') }}
                                                    {{ csrf_field() }}
