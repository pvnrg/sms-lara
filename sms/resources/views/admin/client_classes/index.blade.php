...
@section('title','client_classes')

@section('pageTitle','client_classes')
        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    ...
                    @if(Auth::user()->can('access.client_classes.create'))
                        <a href="{{ url('/admin/client_classes/create') }}" class="btn btn-success btn-sm pull-right"
                            title="Add New Client Classes">
                ...
                <div class="box-body">
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach($client_classes as $item)
                                <tr>
                                    <td>{{ $item->id }}</td>
                                    <td>{{ $item->name }}</td>
                                    <td>
                                        <a href="{{ url('/admin/client_classes/' . $item->id) }}" title="View client_classes">
                                        ...
                                        @if(Auth::user()->can('access.client_classes.edit'))
                                            <a href="{{ url('/admin/client_classes/' . $item->id . '/edit') }}"
                                                title="Edit Skill">
                                        ...
                                        @if(Auth::user()->can('access.client_classes.delete'))
                                            <form method="POST" action="{{ url('/admin/client_classes'.'/'.$item->id) }}" accept-charset="UTF-8" style="display:inline" class = "confirm_delete">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
